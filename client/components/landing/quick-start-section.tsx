    ...
    <section className="bg-surface border-y border-border py-24 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-end justify-between mb-12">
          ...
          <p className="hidden md:block font-mono text-xs text-text-tertiary pb-1">
                ...
                <pre className="font-mono text-[11px] leading-relaxed text-text-secondary bg-surface rounded-[var(--radius)] border border-border p-4 overflow-x-auto whitespace-pre-wrap h-full min-h-[140px]">
                  <code>
                    {step.code.split("\n").map((line, j) => {
                      ...
                      const isString = line.trim().startsWith('"') && line.trim().endsWith('"');
                      const isPrompt = line.startsWith("You:") || line.startsWith("Claude:");
                      return (
                        <span
                          key={j}
                          className={
